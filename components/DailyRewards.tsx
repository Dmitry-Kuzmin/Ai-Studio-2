...
import { Flame, Award, RotateCcw } from 'lucide-react';
import { playEpicRewardSound } from '../services/audioService';
import { RewardOverlay, EffectType } from './RewardOverlay';
...
export const DailyRewards: React.FC<DailyRewardsProps> = ({ currentStreak, hasClaimedToday, onClaim }) => {
  ...
  const handleClaim = () => {
    // Prevent double clicks during animation start
    if (isClaiming) return;
    
    setIsClaiming(true);
    
    // 1. Pick a random effect strictly from the requested types
    const effects: EffectType[] = ['mythic', 'legendary', 'quantum'];
    const randomEffect = effects[Math.floor(Math.random() * effects.length)];
    
    console.log("Triggering Effect:", randomEffect); // Debug for console

    // 2. Play the new EPIC sound
    playEpicRewardSound(); 
    
    // 3. Show Overlay immediately
    setActiveEffect(randomEffect);

    // 4. Handle Logic (Reset fast for testing)
    if (!hasClaimedToday) {
      setTimeout(() => {
        onClaim();
        setIsClaiming(false);
      }, 3000); // Wait for full animation
    } else {
      // Just reset state for testing
      setTimeout(() => {
        setIsClaiming(false);
      }, 3000);
    }
        ...
        <button
          ...
          className={`relative z-10 w-full py-4 rounded-2xl font-bold text-xs tracking-[0.2em] uppercase transition-all duration-300 overflow-hidden group/btn ${
            hasClaimedToday 
              ...
              : 'bg-white text-slate-900 hover:scale-[1.02] active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)] cursor-pointer'
          }`}
        >
          {hasClaimedToday ? (
            <span className="flex items-center justify-center gap-2">
               {isClaiming ? 'Testing...' : (
                 <>
                   <RotateCcw size={14} /> Repetir Efecto (Test)
                 </>
               )}
            </span>
          ) : (
